...
/**
 * 光栅预览 · 地面远处柔边：只让「地面」远处慢慢融进当前天空色（消除天地硬边）。
 * 距离按「离场景中心（头模脚下一带）在地面上有多远」来算，单位是场景世界尺寸。
 * 平行光 / 点光 / 聚光等主光均生效（配置名历史遗留带 DIR，语义已是全灯型地平线柔边）。
 *
 * 通俗调参：
 * - near：多远以内地面保持原样（头模附近别先融天）。数值越大，清晰区越大。
 * - far：多远以外基本融进天空（地平线柔边）。数值越大，变软来得越晚、过渡带越长。
 * - minFactor：最远处还留多少「地面色」（其余混向天空色）。例如 0.05 ≈ 远景约 5% 地面 + 95% 天空，避免完全抹成纯背景时偶发发花。
 *
 * 中间这段（near→far）会平滑过渡，不是一刀切。调天光/天空色时远景会跟着天走。
 * 另：平行光下会额外轻度压暗直射（假衰减辅层）；点/聚光本身已有距离衰减，不再叠那一层。
 */
export const SOLID_DIR_GROUND_DISTANCE_FADE = {
  enabled: true,
  near: 14, // 全亮半径：离中心这么远以内，地面几乎不混天
  far: 42, // 融天半径：离中心这么远以外，按 minFactor 尽量混向天空色
  minFactor: 0.05, // 最远处地面色保留比例（约 5%），其余混天空；略留一点避免纯背景闪感
};
