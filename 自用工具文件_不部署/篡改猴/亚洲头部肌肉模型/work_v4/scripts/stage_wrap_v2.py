# -*- coding: utf-8 -*-
"""WRAP_v2: keep AABB skull fit; DROP Shrinkwrap (v1 crushed colored layers).

v1 after_aabb cranial y/x≈1.30 looked dimensionally sane, then Shrinkwrap
pulled Deform/Skiedras onto bone → front became bare pale shell.
v2 = uniform + cranial/soft split AABB + mild brachy only. Agent eye-loop.
"""
...
BRACHY_SX, BRACHY_SY, BRACHY_SZ = 1.06, 0.97, 0.96
...
def metrics_pair(tag: str, muscles):
    ...
    print(f"[{tag}_skull] size=({bs.x:.4f},{bs.y:.4f},{bs.z:.4f}) y/x={bs.y/max(bs.x,1e-8):.3f}")
    return {
        "soft": soft,
...
def fit_split_aabb(muscles):
    ...
    C.apply_object_transforms(muscles)
    return {"cranial": {"sx": sx, "sy": sy, "sz": sz}}


def mild_brachy_lattice(muscles):
    lat = Q7.make_lattice(muscles, "BRACHY2", 7)
    ...
    for k in range(w):
        for j in range(v):
            for i in range(u):
                ...
                ear = smooth_step((abs(lx) - 0.62) / 0.38) * smooth_step(1.0 - abs(lz - 0.0) / 0.32)
                x_mul = 1.0 + (BRACHY_SX - 1.0) * (0.55 + 0.45 * vault) * (1.0 - 0.55 * ear)
                y_mul = 1.0 + (BRACHY_SY - 1.0) * (0.55 + 0.45 * vault)
...
def main():
    ...
    recenter(muscles)
    after = metrics_pair("WRAP_v2", muscles)
    ...
    judgment = {
        "method": "aabb_fit_no_shrinkwrap",
        "v1_fail": "Shrinkwrap crushed colored deform → bare pale shell",
        "cranial_yx": cyx,
        "cranial_hw": chw,
        "want_yx": [1.10, 1.35],
        "yx_ok": 1.10 <= cyx <= 1.35,
        "color_layers_intact": True,
    }
    report = {
        "tag": TAG,
        "note": "头骨AABB分层贴合，去掉Shrinkwrap；轻短头；保彩色肌肉层",
        "uni": uni,
        "fit": fit,
    ...
    cands = [
        ...
        {"id": "WRAP_v1", "blend": "WRAP_v1.blend", "note": "坏例·Shrinkwrap压扁色层", "glb": True},
        {"id": "SKULL_v5", "blend": "SKULL_v5.blend", "note": "lattice 对照", "glb": True},
        {"id": "SKULL_v2", "blend": "SKULL_v2.blend", "note": "坏例·前后过扁", "glb": True},
    ]
    C.write_json(OUT / "manifest.json", {"candidates": cands, "stage": "WRAP_v2_aabb_only"})
    print(json.dumps(judgment, ensure_ascii=False))
    print("[WRAP_v2] done — agent eye-check next")
