# -*- coding: utf-8 -*-
"""SHELL_v6: from v5 — stronger occiput under orthographic review (no sphere).

Chain: v1 → v3 → v4 → v5 → v6. Compare only with ortho renders + aligned review page.
"""
...
BASE = C.CHECKPOINTS / "SHELL_v5.blend"
TAG = "SHELL_v6"
...
def reshape_lattice(muscles):
    lat = Q7.make_lattice(muscles, "V6_OCCIP_CHEEK", 9)
...
def main():
    ...
    before = metrics_pair("from_v5", muscles)
    print(f"[v6] occip={OCCIP_Y} crown={CROWN_Z} cheek={CHEEK_X}")
    reshape_lattice(muscles)
    recenter_to_skull(muscles)
    after = metrics_pair("SHELL_v6", muscles)
    ...
    print(f"[v6] delta width={width_ratio:.3f} depth={depth_ratio:.3f}")
    ...
    cands = [
        ...
        {"id": "SHELL_v5", "blend": "SHELL_v5.blend", "note": "上版·轻颧宽", "glb": True},
    ...
    print("[SHELL_v6] done — ortho-aligned eye-check")
