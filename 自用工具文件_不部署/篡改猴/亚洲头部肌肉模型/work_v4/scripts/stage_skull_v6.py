# -*- coding: utf-8 -*-
"""SKULL_v6: refine after v5 overshoot.

v5 (sx=1.32 sy=0.88): cranial y/x≈1.21 in band, but eye-check failed —
  top_cranial → over-wide trapezoid / temporal bulge
  front → still tall-narrow face (vault wide ≠ face wide)
  soft_top → ear wings still loud

v6: dial back parietal widen; push zygoma/cheek; stronger ear tuck;
slightly restore AP; lower vault for front.
"""
...
SX_VAULT, SX_CHEEK, SY, SZ, EAR_IN = 1.16, 1.28, 0.93, 0.93, 0.74
...
def metrics_pair(tag: str, muscles):
    ...
    print(f"[{tag}_skull] depth/width={bdw:.3f} (ref only)")
    return {
        "soft": soft,
...
def reshape_lattice(muscles):
    lat = Q7.make_lattice(muscles, "V6_BALANCE", 9)
    ...
    for k in range(w):
        for j in range(v):
            for i in range(u):
                ...
                cheek = smooth_step(1.0 - abs(lz + 0.08) / 0.55) * smooth_step(1.0 - abs(ly + 0.05) / 0.75)
                jaw = smooth_step(1.0 - abs(lz + 0.45) / 0.45) * smooth_step(1.0 - abs(ly + 0.05) / 0.7)
                ear = smooth_step((abs(lx) - 0.48) / 0.52)
                face_w = smooth_step((-ly + 0.1) / 0.7)
                occip = smooth_step((ly + 0.05) / 0.85)

                x_mul = 1.0
                x_mul += (SX_VAULT - 1.0) * vault * (1.0 - 0.35 * ear)
                x_mul += (SX_CHEEK - 1.0) * (0.65 * cheek + 0.45 * jaw) * (1.0 - 0.5 * ear)
                x_mul *= 1.0 + (EAR_IN - 1.0) * ear

                y_mul = 1.0 + (SY - 1.0) * (0.55 + 0.45 * vault + 0.25 * occip) * (1.0 - 0.45 * face_w)
                z_mul = 1.0 + (SZ - 1.0) * (0.6 + 0.5 * vault)
...
def main():
    ...
    print(f"[v6] vault={SX_VAULT} cheek={SX_CHEEK} sy={SY} sz={SZ} ear={EAR_IN}")
    reshape_lattice(muscles)
    recenter_to_skull(muscles)
    after = metrics_pair("SKULL_v6", muscles)
    ...
    judgment = {
        "method": "agent_eye_loop_v6",
        "from_v5_fail": "top trapezoid + front still skinny/tall",
        "cranial_yx": cyx,
        "cranial_hw": chw,
        "want_yx_band": [1.08, 1.25],
        "yx_in_band": 1.08 <= cyx <= 1.25,
        "eye_criteria": [
            "top_cranial: oval not trapezoid/pancake",
            "front: face width reads human, not pinching chin under tall vault",
            "side: occiput round, not flat disc or giraffe",
            "back: vault width matches neck without shelf alien box",
            "soft_top: ears not hammer wings",
        ],
    }
    report = {
        "tag": TAG,
        "note": "人眼自迭代：回退顶骨过宽、加颧宽、收耳、略回前后深、略降颅高",
        "uni": uni,
        "eye_fix": {
            "sx_vault": SX_VAULT,
            "sx_cheek": SX_CHEEK,
    ...
    cands = [
        ...
        {"id": "SKULL_v5", "blend": "SKULL_v5.blend", "note": "上版·顶过宽梯形", "glb": True},
        {"id": "SKULL_v2", "blend": "SKULL_v2.blend", "note": "坏例·前后过扁", "glb": True},
    ]
    C.write_json(OUT / "manifest.json", {"candidates": cands, "stage": "SKULL_v6_balance"})
    print(json.dumps(judgment, ensure_ascii=False))
    print("[SKULL_v6] done — agent eye-check next")
