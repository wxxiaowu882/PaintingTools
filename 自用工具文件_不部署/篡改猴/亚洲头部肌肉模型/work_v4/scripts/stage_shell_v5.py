# -*- coding: utf-8 -*-
"""SHELL_v5: from v4 — more occiput + mild zygoma (Base15 front), never sphere.

Chain: v1 (60) → v3 → v4 → v5. SHELL_v2 rejected forever.
"""
...
BASE = C.CHECKPOINTS / "SHELL_v4.blend"
TAG = "SHELL_v5"

OCCIP_Y = 1.12
CROWN_Z = 1.03
CHEEK_X = 1.07  # mid-face / zygoma only
VAULT_X = 1.00  # must stay ~1 to avoid sphere
...
def reshape_lattice(muscles):
    lat = Q7.make_lattice(muscles, "V5_OCCIP_CHEEK", 9)
    ...
    for k in range(w):
        for j in range(v):
            for i in range(u):
                ...
                # cheek / zygoma band: mid z, forward-mid y, mid |x|
                cheek = (
                    front
                ...
                y_mul = 1.0 + (OCCIP_Y - 1.0) * occip * (0.5 + 0.5 * vault)
                z_mul = 1.0 + (CROWN_Z - 1.0) * occip
                x_mul = 1.0 + (CHEEK_X - 1.0) * cheek * (1.0 - 0.7 * ear)
                x_mul *= 1.0 + (VAULT_X - 1.0) * vault * rear  # stays 1.0
...
def main():
    ...
    before = metrics_pair("from_v4", muscles)
    print(f"[v5] occip={OCCIP_Y} crown={CROWN_Z} cheek={CHEEK_X}")
    reshape_lattice(muscles)
    recenter_to_skull(muscles)
    after = metrics_pair("SHELL_v5", muscles)
    ...
    print(f"[v5] delta width={width_ratio:.3f} depth={depth_ratio:.3f}")
    ...
    judgment = {
        ...
        "not_sphere_gate": width_ratio <= 1.10,
        "agent_pass": False,
    }
    report = {
        "tag": TAG,
        "note": "自v4：再补后枕 + 轻颧宽靠Base15正视；颅顶不加宽",
        "from": "SHELL_v4.blend",
        "params": {"occip_y": OCCIP_Y, "crown_z": CROWN_Z, "cheek_x": CHEEK_X},
        "before": before,
        "after": after,
        "judgment": judgment,
    }
    ...
    cands = [
        ...
        {"id": "SHELL_v4", "blend": "SHELL_v4.blend", "note": "上版·后枕", "glb": True},
