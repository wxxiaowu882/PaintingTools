...
| 阶段 | 行为 |
|------|------|
| 监控 | 约 2 秒一轮；用量上限 / 恢复会话失败 → 换号 |
| Connection Error | 关 tip → 输入「刚才网络断了，现在请继续」并 Ctrl+Enter（不换号） |
| 换号打开工程后 | 最多等约 20 秒 Connection Error；命中则网络续写，否则清 tip 后发「请继续你的工作」 |
| 宽限内（打开工程后 2 分钟） | 用量 tip 忽略；Connection Error 仍走网络续写 |
| 换号中 | 忽略新 tip，避免连点 |
| 换号前 | 对 Cursor「全部保存」 |
