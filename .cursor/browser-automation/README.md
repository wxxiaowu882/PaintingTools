...
21. **头部造型规律样板关标注（消费端 Solid）**：`sandbox=headform` 的 101/201/102/103…。消费端需 `window.__solidHost`（camera/sceneGroup/THREE）才能 `page.evaluate` 射线。统合只合并 `^\d+_.+\.json$`。自动化建议 URL 加 **`skipPerf=1`**；`switchScene` 后须等到 sceneGroup 有 mesh 再射线（否则拾取为空或截图空景）。射线步进宜粗（如 u+=0.004），过密会卡死 `page.evaluate`。
    ...
    - **光位场景文案口径（201/202 手调吸收，2026-09-12）**：
      1. 开场：本场景只看这一档光下交界怎么走；交界跟「光线和头的夹角」有关，**和观察角度无关**——可转场景换角度看。
      2. 单独一段：**请不要改光线**（方向/高低等），否则影响教学观感；误改点「重置」。
      3. 先看表象（环形未接 / 伦勃朗接到+三角亮…）并对照相邻场景。
      4. 曲线串主交界 + 圆标钉结构点；可一句反推结构。
      5. 点位表（主线 / 支线）。
      6. 收尾提示：可点击**操作面板**上的眼睛图标隐藏标注，更干净地观察交界。
      - 学员向一律「本场景 / 上一场景 / 下一场景」，禁止「本关」。
    ...
    - **背面光 · 背侧交界（207 / L07）**：光在头后（约 **az275 / el9**），侧后取景；脸大体暗、背侧亮边。手调：**①颅顶结节 → ②颅侧结节 → ③耳朵 → ④颞骨乳突 → ⑤胸锁乳突肌后缘**；补充 **Ⓐ颌后沟**（下颌枝与胸锁乳突肌之间暗沟）、**Ⓑ下颌亮面**（暗示从背面看到的下颌厚度）。旧名「逆光 · 轮廓光（Rim）」已弃。脚本 `headform-anno-207.js`（打底参考；权威以 Create 手调为准）。
    - **底光 · 显凹（208 / L08）**：近下方、略偏前底光（约 **az90 / el−74**；比正下死底光稍靠前）；正脸取景。手调：**①眉弓下／眶窝 ②颧→鼻唇沟（左右） ③鼻底 ④上唇与嘴角 ⑤下唇白脊 ⑥颏前缘／颏凸隆 ⑦鼻唇沟（左右） ⑧颊侧→下颌底（左右）**。脚本 `headform-anno-208.js`（打底参考；权威以 Create 手调为准）。
