...
21. **头部造型规律样板关标注（消费端 Solid）**：`sandbox=headform` 的 101/201/102/103…。显示名：**【外轮廓规律】**（101–107）／**【交界线规律】**（201–209）；勿再写【视角】／【光位】。学员文案口径以 **102** 为样板：抓重点、少废话；结构点=建外轮廓／明暗交界（内轮廓）的关键点位。消费端需 `window.__solidHost`……
    ...
    - **大半侧（103）**：比侧前更侧、未到全侧（约 50–60°）；主课改为**额—鼻—唇—颏剪影线**（与 101 颧颊课区分）。用户手改口径：凸 **1–6**（含上下唇白脊 + 颏肌）、凹 **A–E**（鼻根 / **眼裂** / 鼻唇角 / **唇缝** / **颏唇沟**）——唇与眼裂要细分，勿只钉鼻头与颏。脚本：`headform-anno-103*.js`。
    - **全侧面（104）**：正侧面宜略偏正、略抬相机（用户手调约 80° 非死 90°）。凸点精简为 **1–5：眉弓→鼻头→上/下唇白脊→颏**（不再单独编号额结节）；凹 **A–E：眉弓上缘 / 鼻根 / 鼻唇角 / 唇缝 / 颏唇沟**（全侧去掉「眼裂」）。种子可迁自 103，但须按全侧再手调。颅顶/枕骨延后。
    - **侧后（105A/105B）**：拆成见鼻 / 隐鼻两关对照；主钉过肩视角下的脸侧外轮廓（非后脑主课）。文件名允许 `105A_` 字母后缀；统合正则用 `^\d+[A-Za-z]?_.+\.json$`。脚本：`headform-anno-105*.js` / `headform-shot-105ab.js`。
    - **背面（106）**：正背面远摄收紧；点位宜少（用户手调约 1–4：颅顶/颅侧/下颌角后/胸锁后缘）；与正面前后相离。脚本：`headform-anno-106-back.js`。
    - **顶视（107）**：真顶视远摄（用户手调约 fov6、相机抬很高）；主课颅顶最大范围；点位 **1–5：鼻头 / 眉弓（整段前额轮廓）/ 颧弓（前半最宽）/ 颅侧结节（后半最宽）/ 枕后突隆**。脚本：`headform-anno-107.js`。
    - **Loop 光位（201 / L01）**：圆标钉**交界结构点**，不是「亮面/环影」现象区名。手调深化后主线约 **1–7：颞线→眉峰→眶外角→颧突隆→口轮匝肌侧缘→颏结节→颏底**；支线 **A 眼睑交界**、**B 眉头→鼻底**。表象钉子仍是「鼻旁交界未接颊侧大交界」。可用 `dashedLines`（`kind: bezier`）串主交界。脚本：`headform-anno-201-struct.js`（早期种子）。
      - **手调种子（相机/光）**：约 `fov:14`、`zoom:0.58`；光 **az≈58 / el≈30 / size≈11**；`lightIndicatorEnabled: false`。具体点位/曲线以用户最新手调 JSON 为准。
      - **左右**：az≈55–60 → 光在世界 **+X**，主交界在 **−X**。截图前 `stopRender()` + 藏 loader。
      - **用词**：正侧交界下端统一「**颏结节**」（下巴）；书页部分图注作「颊结节」属混用，知识库摘要已订正。
    - **伦勃朗（202 / L02）**：表象钉子「鼻侧交界**接到**颊侧大交界 + 暗颊三角亮」；可另标现象区名「伦勃朗三角」。主线点位可沿额结节—眉—鼻—唇—颏一路加密（手调约 1–11）；注意 **①过额结节的竖交界 ≠ 颞线**。起步光约 **az 34–40 / el 44–48**（常用 az36/el46）。脚本：`headform-anno-202.js` / `headform-anno-202b.js`。
    - **交界线场景文案口径（2026-09-14，对齐 102）**：
      1. 开场：本场景研究对象是某档光下的明暗交界线（也可叫内轮廓）；勿改光，误改点重置；可转视角看交界。
      2. 看标注：圆标／短曲线钉在交界经过的关键结构点或线段上——这些点位决定这一档光的光影交界形态。
      3. 一句框架：个体凹凸不同，先有点位框架再强化／弱化。
      4. 点位表；收尾可藏标注。
      - 学员向一律「本场景／上一场景／下一场景」，禁止「本关」；禁止 az/el 等制作参数。
    - **（旧）光位场景文案口径（201/202 手调吸收，2026-09-12）**：已由上条 102 口径取代；技术备忘仍可参考锁光可转视角。
    - **蝴蝶光（203 / L03）**：正前略高 **az≈90 / el≈58**。用户手调后改为**标交界线段**（非点位）：①眉弓（眶外角→眉峰→眉头→鼻根侧）②下眼睑 ③鼻底/侧 ④上唇上缘与嘴角上缘 ⑤颏唇沟上缘与降下唇肌侧面 ⑥下巴下缘偏上一圈 ⑦颧骨上 ⑧脸颊上（颊凹下缘）。虚线只贴真实交界；圆标仅编号。文案须写「标交界线、不钉零散点」。
    - **正侧 Split（204 / L04）**：风格**仿 202 长交界**（勿仿 203 多段短线）。打底光约 **az≈10 / el≈20 / size≈6 / intensity≈2.35**。主线 **1–7** + 眼 **A/B** + 贝塞尔。钉点用 `headform-anno-204-edge.js`（像素交界 aim≈峰值×0.72；暗坑向亮逃逸；禁 `|x|` 中线拉扯；主标 `dx` 偏受光侧）。投影验：`headform-204-proj-verify.js`。已手调。
    - **正前光 · 头侧交界（205 / L05）**：知识库口径——正前光下光照边际≈正面视点头侧交界。手调结构：**交界1** ①颅顶结节（发下头骨）→②颅侧结节（发下头骨）→③颧骨弓隆起→④颧突隆→⑤颊转角（咬肌上、非下颌角）→⑥颏结节；**交界2** ⑦→⑧颞线/眉峰→⑨眶外角。场景名勿写「柔亮少影」（本关研究交界，非固定柔亮观感）。脚本 `headform-anno-205.js` / `headform-anno-205-vision.js`（打底参考；权威以 Create 手调为准）。
    - **顶前光 · 顶前交界（206 / L06）**：比 203 蝴蝶光更顶更正（约 **az90 / el79**），仍是**偏前顶光**（非正顶）。**正脸取景**。手调七段短线：①眉弓 ②颧骨上 ③鼻底 ④上唇白脊上 ⑤下唇白脊 ⑥下巴前缘 ⑦鼻唇沟。打底脚本 `headform-anno-206-fronttop.js`（权威以 Create 手调为准）。
    - **底前光 · 底前交界（207 / L07）**：近下方、略偏前底前光（约 **az90 / el−74**）；正脸取景。手调：**①眉弓下／眶窝 ②颧→鼻唇沟（左右） ③鼻底 ④上唇与嘴角 ⑤下唇白脊 ⑥颏前缘／颏凸隆 ⑦鼻唇沟（左右） ⑧颊侧→下颌底（左右）**。旧名「底光 · 显凹」。脚本 `headform-anno-208.js`（打底参考；权威以 Create 手调为准）。
    - **背侧光 · 背侧交界B（208 / L08）**：侧后光（约 **az235 / el14**）；侧后取景。手调交界：**①–②颞线 → ②眉峰 → ③眶外角 → ④颧突隆 → ⑤–⑥咬肌 → ⑥咬肌前切迹 → ⑦颏转角 → ⑧胸锁乳突肌**。对照下一场景 A。脚本 `headform-anno-209.js`（打底参考；权威以 Create 手调为准）。
    - **背面光 · 背侧交界A（209 / L09）**：光在头后（约 **az275 / el9**），侧后取景；脸大体暗、背侧亮边。手调：**①颅顶结节 → ②颅侧结节 → ③耳朵 → ④颞骨乳突 → ⑤胸锁乳突肌后缘**；补充 **Ⓐ颌后沟**、**Ⓑ下颌亮面**。对照上一场景 B。脚本 `headform-anno-207.js`（打底参考；权威以 Create 手调为准）。
    - **Create 导出**：生产端存盘常会**丢掉 `id` 与 meta.line/slot/status**；手调后须回写（如 L01/L02），自动化查找勿只靠 id。
    - **关标注截图**：只设 `showAnnotations=false` 不够；须调用已暴露的 `window.toggleAnnotations()`（内部的 `applySolidAnnotationOverlayLayers` 未挂到 window），否则 light-only 与 annotated 可能同图。
    - **文案**：`meta.detail`（场景说明）必须面向学员——禁止写「不连虚线 / 柔光 / 种子迁自 / 打底」等制作备注。
    - **虚线 / 贝塞尔**：光位关可用曲线串交界；制作侧备注勿进学员文案。
    - **光方位备忘**：Solid UI **90=正面光，0 / 180=侧光**；笛卡尔：`x∝cos(az)`、`z∝sin(az)`，故 **az0→+X、az90→+Z（正前）**。钉交界时以画面明暗为准。
    - **光栅暗部反光（2026-09-13）**：无 `materials` 的 GLB 会被 GLTFLoader 套默认 `metalness=1`，漫反射（含 SH 地面反光）进不了暗部。已在 stamp 时按指纹纠正；SH 须在 `onBeforeCompile` 前建好 `_solidShUni`，否则首轮 `shMix` 停在 0。验证：`metal≈0` + 探针后 `shMix≈0.47`（勿只靠肉眼“仍很暗”）。
