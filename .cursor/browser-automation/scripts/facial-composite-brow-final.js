...
const outDir = path.join(__dirname, '..', 'runs', `${stamp}-brow-final`);
...
(async () => {
  ...
  const JOBS = [
    { text: '睑眉沟', u: 0.5, v: 0.36, box: [ix - 0.012, ix + 0.012, iy + 0.008, iy + 0.018, iz - 0.01, iz + 0.004] },
    { text: '眉头', u: 0.58, v: 0.32, box: [nx - 0.006, nx + 0.010, iy + 0.012, iy + 0.022, iz - 0.006, iz + 0.008] },
    { text: '眉峰', u: 0.42, v: 0.30, box: [wx + 0.004, (wx + ix) / 2 + 0.004, iy + 0.014, iy + 0.024, iz - 0.01, iz + 0.006] },
    { text: '眉毛最浓处', u: 0.48, v: 0.32, box: [ix - 0.014, ix + 0.002, iy + 0.012, iy + 0.022, iz - 0.008, iz + 0.004] },
    { text: '眉弓隆起', u: 0.50, v: 0.28, box: [ix - 0.012, ix + 0.008, iy + 0.016, iy + 0.028, iz - 0.012, iz + 0.002] },
  ];
  ...
  for (const name of ['眉头', '眉峰', '睑眉沟', '上眼睑', '外眦', '内眦', '虹膜']) {
    const pt = by[name];
  ...
  const overviewJson = path.join(outDir, '_full_eye_cam.json');
  fs.writeFileSync(
    overviewJson,
    JSON.stringify({ ...data, timestamp: Date.now(), camera: { ...EYE_CAM } }, null, 2)
  );
  await page.setInputFiles('#file-input', overviewJson);
  ...
  await page.waitForTimeout(500);
  await page.locator('#workbench-viewer').screenshot({ path: path.join(outDir, 'qa_eye_all.png') });
