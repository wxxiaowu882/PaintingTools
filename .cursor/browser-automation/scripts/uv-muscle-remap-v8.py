...
# -*- coding: utf-8 -*-
"""
Euro→V8 肌肉贴点：用共享 Deform UV 岛锁定肌肉，
在同 UV 邻域内再选靠近原版世界坐标（保留左右）的顶点。
写回 03 肌肉详解_黄种人女V8.json，不覆盖原版。
"""
...
REPORT = REPO / ".cursor/browser-automation/runs/_uv-muscle-remap-report.json"
...
def load_deform(path: Path):
    ...
    for i, n in enumerate(gltf["nodes"]):
        ...
        return {"name": name, "pos": wpos, "nrm": wnrm, "uv": uv}
    raise RuntimeError("Deform not found")
...
def nearest_pos(positions, point):
...
def main():
    ...
    print(f"euro={len(euro['pos'])} v8={len(v8['pos'])}")

    # UV grid for candidate gathering
    res = 128
    grid = defaultdict(list)
    for i, (u, v) in enumerate(v8["uv"]):
        grid[(int(u * res) % res, int(v * res) % res)].append(i)

    def uv_candidates(u, v, max_uv=0.012, want=400):
        ...
        if not cands:
            # full scan fallback
            for j, (uu, vv) in enumerate(v8["uv"]):
                d = math.hypot(uu - u, vv - v)
                if d <= max_uv * 3:
                    cands.append((d, j))
        cands.sort()
        # unique indices keep best uv
        seen = {}
        for d, j in cands:
            if j not in seen:
                seen[j] = d
        return list(seen.items())  # (j, uvd)

    by_out = {p["text"]: p for p in out["pointsData"]}
    results = []
    ...
    for sp in src["pointsData"]:
        ...
        ei, ed = nearest_pos(euro["pos"], spos)
        u, v = euro["uv"][ei]
        cands = uv_candidates(u, v)
        if not cands:
            results.append({"text": text, "ok": False, "error": "no uv cand"})
            continue

        # Score: stay on UV island + near original world pos (laterality) + normal hemisphere
        best = None
        for j, uvd in cands:
            p = v8["pos"][j]
            n = v8["nrm"][j]
            # Prefer matching source laterality: if src x>0 prefer p.x>0
            side_pen = 0.0
            if abs(spos[0]) > 0.01:
                if spos[0] * p[0] < 0:
                    side_pen = 0.04
            ...
            # UV exactness weight small; spatial dominates within island
            score = spatial + side_pen + uvd * 0.5
            ndot = n[0] * snorm[0] + n[1] * snorm[1] + n[2] * snorm[2]
            if ndot < 0:
                score += 0.01
            if best is None or score < best[0]:
                best = (score, j, uvd, spatial, ndot)
        ...
        by_out[text]["pos"] = fmt_vec(pos)
        by_out[text]["norm"] = fmt_vec(nrm)
        results.append(
            {
                "text": text,
                "ok": True,
                "euroMeshDist": round(ed, 5),
                "uv": [round(u, 5), round(v, 5)],
                "uvDist": round(best[2], 6),
                "spatialToSrc": round(best[3], 5),
                "cand": len(cands),
                "pos": by_out[text]["pos"],
                "norm": by_out[text]["norm"],
                "srcPos": sp["pos"],
            }
        )
    ...
    for r in results:
        print(
            f"{'OK' if r.get('ok') else 'FAIL':4s} {r['text']:12s} spat={r.get('spatialToSrc')} uvd={r.get('uvDist')} pos={r.get('pos')}"
        )
