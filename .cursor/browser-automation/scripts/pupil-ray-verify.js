...
(async () => {
  ...
  const info = await page.evaluate(async () => {
    ...
    const occ = app.scene?.userData?._pupilOccluders || root?.userData?._pupilOccluders || [];
    const proj = (obj) => {
      const v = new THREE.Vector3();
      obj.getWorldPosition(v);
      v.project(cam);
      return { nx: v.x * 0.5 + 0.5, ny: -v.y * 0.5 + 0.5, wp: v.clone() };
    };
    ...
    const headCenter = new THREE.Box3().setFromObject(root).getCenter(new THREE.Vector3());
    const headDist = cam.position.distanceTo(headCenter);
    const rays = targets.map(({ nx, ny }) => {
      ndc.set(nx * 2 - 1, -(ny * 2 - 1));
      raycaster.setFromCamera(ndc, cam);
      const p = new THREE.Vector3();
      raycaster.ray.at(headDist, p);
      const q = p.clone().project(cam);
      return {
        target: { nx, ny },
        rayAt: { nx: q.x * 0.5 + 0.5, ny: -q.y * 0.5 + 0.5 },
        dist: headDist,
      };
    });
    return {
      sprites: occ.map((o, i) => ({ i, ...proj(o) })),
      rays,
      headDist,
