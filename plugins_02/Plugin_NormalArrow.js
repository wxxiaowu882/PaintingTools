import * as THREE from 'three'; window.normArrowList = []; window.normArrowCounter = 0; window.NormalArrowManager = { selectedId: null, onSceneHit: function(context) { // 【核心精准修复】：法线箭头专属，必须严格校验当前是否为 'normal-arrow' 模式！
    ...
    const arrowData = { id: id, targetUUID: targetObj.uuid, anchorObj: anchor, color: color, isOccluded: false }; window.normArrowList.push(arrowData); this.buildDOM(arrowData); window.needsUpdate = true; window.lightMoved = true;
    this.selectedId = id; this.highlightSelected(); } catch(e) { console.error(e); } }, buildDOM: function(data) { const svg = document.getElementById('norm-arrow-svg'); const ns = "http://www.w3.org/2000/svg";
    // 选择发光层
    ...
    /* 【优化2 & 3】：移除 e.stopPropagation() 允许事件穿透旋转场景；移除模式拦截随时可点；互斥取消其他标注 */
    if (window.AnnotationManager) { window.AnnotationManager.selectedId = null; window.AnnotationManager.highlightSelected(); } if (window.Polygon3DManager) { window.Polygon3DManager.selectedId = null; window.Polygon3DManager.highlightSelected(); }
    window.NormalArrowManager.selectedId = data.id; window.NormalArrowManager.highlightSelected(); }); /* 移除click拦截 */ }); }, updateScreenPositions: function(camera) {
    ...
    if(a.baseScale) arrowData.baseScale = a.baseScale; window.normArrowList.push(arrowData); this.buildDOM(arrowData); }); } }; // 独立监听 Delete 键删除功能
    window.addEventListener('keydown', e => { if (e.key === 'Delete' || e.key === 'Backspace') {
    if (document.activeElement && document.activeElement.tagName === 'INPUT') return; const id = window.NormalArrowManager.selectedId; if (id !== null) { const idx = window.normArrowList.findIndex(a => a.id === id);
    if (idx > -1) { const data = window.normArrowList[idx]; if(data.anchorObj && data.anchorObj.parent) data.anchorObj.parent.remove(data.anchorObj); if(data.svgGlowPath) data.svgGlowPath.remove(); if(data.svgPath) data.svgPath.remove();
    if(data.svgHitPath) data.svgHitPath.remove(); if(data.svgFace1) data.svgFace1.remove(); if(data.svgFace2) data.svgFace2.remove(); if(data.svgFace3) data.svgFace3.remove(); window.normArrowList.splice(idx, 1);
