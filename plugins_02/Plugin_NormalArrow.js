import * as THREE from 'three'; window.normArrowList = []; window.normArrowCounter = 0; window.NormalArrowManager = { selectedId: null, onSceneHit: function(context) { // 【核心精准修复】：法线箭头专属，必须严格校验当前是否为 'normal-arrow' 模式！
    ...
    if (!document.getElementById('norm-arrow-style-inject')) { const style = document.createElement('style'); style.id = 'norm-arrow-style-inject'; style.innerHTML = `
        .norm-arrow-label { position:absolute; transform:translate(0,-50%); pointer-events:auto; font-family:'Inter', ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto; border:1px solid rgba(255,255,255,0.35); color:#fff; padding:4px 8px; font-size:11px; font-weight:700; white-space:pre-wrap; max-width:260px; user-select:none; border-radius:2px; transition:opacity 0.2s; box-shadow:0 4px 10px rgba(0,0,0,0.5); cursor:pointer; }
