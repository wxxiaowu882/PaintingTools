import * as THREE from 'three'; window.normArrowList = []; window.normArrowCounter = 0; window.NormalArrowManager = { selectedId: null, onSceneHit: function(context) { // 【核心精准修复】：法线箭头专属，必须严格校验当前是否为 'normal-arrow' 模式！
    ...
    normalArrows.push({ id: nData.id, color: nData.color, text: nData.text, labelVisible: nData.labelVisible !== false, localPos: [parseFloat(c.position.x.toFixed(4)), parseFloat(c.position.y.toFixed(4)), parseFloat(c.position.z.toFixed(4))], localNormal: norm, baseDist: nData.baseDist, baseScale: nData.baseScale });
    ...
    if (!document.getElementById('norm-arrow-style-inject')) { const style = document.createElement('style'); style.id = 'norm-arrow-style-inject'; style.innerHTML = `
        .norm-arrow-label { position:absolute; transform:translate(0,-50%); pointer-events:auto; font-family:'Inter', ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto; border:1px solid #0df; color:#fff; padding:4px 8px; font-size:11px; font-weight:700; white-space:pre-wrap; max-width:260px; user-select:none; border-radius:2px; transition:opacity 0.2s; box-shadow:0 4px 10px rgba(0,0,0,0.5); cursor:pointer; }
    ...
    if (data.domLabel) { data.domLabel.style.borderColor = data.color; data.domLabel.style.backgroundColor = 'rgba(0,0,0,0.65)'; }
    ...
    } catch(e) { return hex; } }, createNormalArrow: function(targetObj, worldPoint, worldNormal) { try { this.ensureDOM(); window.normArrowCounter++;
    ...
    const arrowData = { id: id, targetUUID: targetObj.uuid, anchorObj: anchor, color: color, text: '法线', labelVisible: true, isOccluded: false }; window.normArrowList.push(arrowData); this.buildDOM(arrowData); window.needsUpdate = true; window.lightMoved = true;
    ...
    if(a.baseScale) arrowData.baseScale = a.baseScale; arrowData.text = (a.text !== undefined && a.text !== null) ? String(a.text) : '法线'; arrowData.labelVisible = a.labelVisible !== false; window.normArrowList.push(arrowData); this.buildDOM(arrowData); }); } }; // 独立监听 Delete 键删除功能
