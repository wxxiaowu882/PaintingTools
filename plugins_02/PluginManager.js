window.PluginManager = { plugins: [], // 【1. 核心状态隔离区 Core.State】统一接管 window.currentEditorMode 等零散状态
    ...
    if (eventName === 'onBeforePointerDown') { const _d = window.hwLog; if (_d) _d(`[ColorSampleDbg] onBeforePointerDown 拦截 ← 插件「${p.name}」 (PluginManager.State.currentMode=${this.State.currentMode})`); } } }
