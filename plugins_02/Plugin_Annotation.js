...
window.annoDataList = []; window.annoCounter = 0;
window.AnnotationManager = {
        ...
        onSceneHit: function(context) {
            if (!context.event.shiftKey) return; // 属于法线箭头的操作，忽略
        ...
        onKeyUp: function(event) {
            if (this.isPlacing && (event.key === 'Alt' || event.key === 'Shift')) {
                this.isPlacing = false; this.activeData = null;
                if (this._cachedControls) this._cachedControls.enabled = true;
                if (window.showToast) window.showToast("引线已放置。");
                window.needsUpdate = true;
            }
        },
        onBeforePointerDown: function(e) { if (this.isPlacing) return true; }, // 拦截原生点击
        ...
        onClearScene: function() {
            ...
            window.annoDataList.forEach(data => {
                const dom = document.getElementById('dom_' + data.id); if (dom) dom.remove();
                if (data.svgPath) data.svgPath.remove(); if (data.svgGlowPath) data.svgGlowPath.remove(); if (data.svgCircle) data.svgCircle.remove();
            });
            window.annoDataList = []; this.selectedId = null; this.isPlacing = false; this.activeData = null;
        },

        // 【数据流生命周期】：接管 JSON 反序列化读取 (Core.IO)
        onLoadItem: function(ctx) { if(ctx.itemData.annotations) this.restoreAnnotations(ctx.obj, ctx.itemData.annotations); },
        onLoadGround: function(ctx) { if(ctx.sceneData.groundAnnotations) this.restoreAnnotations(ctx.obj, ctx.sceneData.groundAnnotations); },
        ...
        extractSaveData: function(obj) {
            ...
            obj.children.forEach(c => { 
                if(c.name && c.name.startsWith('anno_')) { 
        ...
        onDrawSnapshot: function(context) {
            ...
            window.annoDataList.forEach(data => {
                ...
                ctx.strokeStyle = data.color; ctx.lineWidth = 1.5; ctx.beginPath(); ctx.moveTo(tx, ty); ctx.lineTo(tmidX, ty1); ctx.lineTo(tx1, ty1); ctx.stroke();
                ctx.fillStyle = data.color; ctx.beginPath(); ctx.arc(tx, ty, 3, 0, Math.PI * 2); ctx.fill();
                ctx.font = '11px Inter, sans-serif'; const textWidth = ctx.measureText(data.text).width; const boxW = textWidth + 16, boxH = 20;
                ctx.fillStyle = window.AnnotationManager.getDarkBg(data.color); ctx.fillRect(tx1 - boxW/2, ty1 - boxH/2, boxW, boxH);
                ctx.strokeStyle = data.color; ctx.lineWidth = 1; ctx.strokeRect(tx1 - boxW/2, ty1 - boxH/2, boxW, boxH);
                ctx.fillStyle = '#ffffff'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(data.text, tx1, ty1);
            });
        },

        // 【新增核心】：根据传入的十六进制颜色，自动计算出对应的深色系背景（亮度降至20%，带0.85透明度）
    ...
    highlightSelected: function() {
        ...
        if (picker) {
            picker.disabled = false; picker.style.opacity = '1'; picker.style.cursor = 'pointer';
        }
    },
    
    ensureDOM: function() {
        if (!document.getElementById('anno-style-inject')) {
            const style = document.createElement('style'); style.id = 'anno-style-inject';
            // 从 CSS 里去掉了固定黑背景，改为通过内联样式动态注入
            style.innerHTML = `
        ...
        if (!document.getElementById('anno-layer')) {
            const layer = document.createElement('div'); layer.id = 'anno-layer'; layer.innerHTML = '<svg id="anno-svg"></svg>';
    ...
    createLeader: function(targetObj, worldPoint, worldNormal) {
        try {
            ...
            window.annoDataList.push(annoData); this.buildDOM(annoData); window.needsUpdate = true; window.lightMoved = true;
    ...
    buildDOM: function(data) {
        ...
        const div = document.createElement('div'); div.className = 'anno-dom anno-leader-label'; div.id = 'dom_' + data.id; div.innerText = data.text; 
        ...
        svg.appendChild(circle); data.svgCircle = circle; data.domEl = div;
    },
    
    updateScreenPositions: function(camera) {
        if(window.annoDataList.length === 0) return; 
        
        // 【工业级性能优化】：懒加载全局向量池，彻底消灭 new 和 clone，拒绝 GC 垃圾回收造成的卡顿死锁
        if (!this._poolInit) {
        ...
        window.annoDataList.forEach(data => {
            ...
            if (!data.baseDist) {
                // 惰性初始化：在刚创建的瞬间，记录当时的相机距离和模型缩放，作为“物理基因”
            ...
            // 2. 文本缩放比例 (textScale): 永远锁定为 1.0，保证文字在任何缩放级别下大小不变，永远清晰可读
            const textScale = 1.0;
            ...
            // SVG 折线的绘制偏移量，挂载 3D 物理透视缩放 (lineScale)
            const scaledDx = data.dx * lineScale;
            const scaledDy = data.dy * lineScale;
            
            data.screenX = x; data.screenY = y;
            data.scaledDx = scaledDx; data.scaledDy = scaledDy;
    ...
    clearAll: function() { 
        ...
        const svg = document.getElementById('anno-svg'); if(svg) svg.innerHTML = ''; 
...
// 【核心修复】：实时监听 UI 拾色器变动，直接染色当前选中的引线（外框、背景及 SVG 连线）
