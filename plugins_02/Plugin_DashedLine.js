...
window.dashedLineList = [];
window.dashedLineCounter = 0;

window.DashedLineManager = {
    pluginId: 'Plugin_DashedLine',
    selectedId: null,
    isDrawing: false,
    currentLine: null,
    _lastAddPos: new THREE.Vector3(),

    ensureDOM: function() {
        if (!document.getElementById('dashed-line-layer')) {
            const layer = document.createElement('div');
            layer.id = 'dashed-line-layer';
            // z-index: 48 确保在底层，不干扰法线和引线点击
            layer.style.cssText = 'position: absolute; top: 0; left: 0; width: 100vw; height: 100vh; pointer-events: none; z-index: 48; overflow: hidden;';
            layer.innerHTML = '<svg id="dashed-line-svg" style="width: 100%; height: 100%; pointer-events: none;"></svg>';
                ...
                colorPicker.addEventListener('input', e => {
                    if (this.selectedId !== null) {
                        ...
                        if (data) {
                            ...
                            if (data.domEl) {
                                data.domEl.dataset.color = data.color;
                                data.domEl.style.borderColor = data.color;
                            }
    ...
    startLine: function(anchorParent, worldPos, worldNormal) {
        this.ensureDOM();
        this.isDrawing = true;
        ...
        const data = {
            ...
            anchorObj: anchorObj,
            color: '#00d2ff',
            text: "线段 " + (++window.dashedLineCounter),
            points: [], // 纯内存数据阵列: { localPos, localNormal }
            midIndex: 0,
            isOccluded: false,
            lastDStr: ''
        };
        ...
        this._lastAddPos.copy(worldPos);

        this.addPoint(worldPos, worldNormal, true);
        this.buildSVG(data);
        return data;
    },

    addPoint: function(worldPos, worldNormal, force = false) {
        if (!this.isDrawing || !this.currentLine || !this.currentLine.anchorObj) return;
        
        // 【距离检测防刷屏】：世界距离大于 0.015 (1.5厘米) 才记录，防止点位过于密集挤爆内存
        if (!force && this._lastAddPos.distanceTo(worldPos) < 0.015) return;

        this._lastAddPos.copy(worldPos);
        const localPos = this.currentLine.anchorObj.worldToLocal(worldPos.clone());
    ...
    buildSVG: function(data) {
        const svg = document.getElementById('dashed-line-svg');
        const ns = "http://www.w3.org/2000/svg";
        
        const glowPath = document.createElementNS(ns, "path");
        glowPath.setAttribute("fill", "none"); glowPath.setAttribute("stroke", data.color);
        glowPath.setAttribute("stroke-width", "5"); glowPath.setAttribute("opacity", "0.2");
        glowPath.style.pointerEvents = "none";
        
        const path = document.createElementNS(ns, "path");
        path.setAttribute("fill", "none"); path.setAttribute("stroke", data.color);
        path.setAttribute("stroke-width", "2.5"); path.setAttribute("stroke-dasharray", "6, 6");
        ...
        hitPath.setAttribute("stroke-width", "20"); hitPath.style.pointerEvents = "auto";
    ...
    highlightSelected: function() {
        ...
        if (this.selectedId !== null) {
            ...
            const data = window.dashedLineList.find(a => a.id === this.selectedId);
            if (data) {
                if (data.domEl) data.domEl.style.boxShadow = `0 0 10px ${data.color}`;
                if (data.svgPath) data.svgPath.setAttribute("stroke-width", "4");
                if (data.svgGlowPath) data.svgGlowPath.setAttribute("opacity", "0.5");
            }
        }
    },
    ...
    deleteSelected: function() {
        if (this.selectedId !== null) {
            this.deleteLine(this.selectedId);
            this.selectedId = null;
        }
    },

    deleteLine: function(id) {
        const idx = window.dashedLineList.findIndex(a => a.id === id);
        if (idx > -1) {
            const data = window.dashedLineList[idx];
            ...
            window.dashedLineList.splice(idx, 1);
            window.needsUpdate = true;
        }
    },

    updateScreenPositions: function(camera) {
        if(window.dashedLineList.length === 0) return;
        
        // 【核心性能护城河】：零对象分配池，斩断 GC 回收的性能卡顿
        if (!this._poolInit) {
            this._tempV = new THREE.Vector3(); this._normalMatrix = new THREE.Matrix3(); 
            this._viewDir = new THREE.Vector3(); this._currentWorldNormal = new THREE.Vector3();
            this._poolInit = true;
        }

        window.dashedLineList.forEach(data => {
            if(!data.anchorObj || data.points.length === 0) return;

            if (data.anchorObj.parent) {
            ...
            for (let i = 0; i < data.points.length; i++) {
                ...
                this._tempV.copy(pt.localPos);
                data.anchorObj.localToWorld(this._tempV);

                // 【点乘遮挡剔除】：用超轻量级数学算法替代 DOM 获取和射线，只要有一点转到背面，整条线变半透明幽灵状态
                if (!isOccluded && data.anchorObj.parent) {
                    this._currentWorldNormal.copy(pt.localNormal).applyMatrix3(this._normalMatrix).normalize();
                    this._viewDir.copy(camera.position).sub(this._tempV).normalize();
                    if (this._currentWorldNormal.dot(this._viewDir) < -0.05) {
                        isOccluded = true;
                    }
                }
                ...
                if (!isNaN(x) && !isNaN(y) && !isBehind) {
                    dStr += `${dStr.length === 0 ? 'M' : 'L'} ${x} ${y} `;
                    if (i === data.midIndex && !this.isDrawing) {
                        midX = x; midY = y; midVisible = true;
                    }
                }
            }

            // 【脏检查】：只在画面像素级变动时才刷新 DOM，节省 90% 性能
            if (data.lastDStr !== dStr) {
            ...
            if (data.lastOccluded !== isOccluded) {
                const opacity = isOccluded ? "0.2" : "0.9";
                if (data.svgPath) data.svgPath.setAttribute("opacity", opacity);
            ...
            if (data.domEl) {
                if (midVisible) {
                    data.domEl.style.display = 'block';
                    data.domEl.style.left = midX + 'px';
                    data.domEl.style.top = (midY - 15) + 'px'; 
                } else {
                    data.domEl.style.display = 'none';
                }
            }
        });
    },
...
if (window.PluginManager) {
    window.PluginManager.register('DashedLine', window.DashedLineManager);
}

window.DashedLineManager.onUpdate = function(context) {
