import * as THREE from 'three'; window.PerfTester = { isTesting: false, testDone: false, state: 'init', idleFrames: 0, maxIdleFrames: 3, targetSamples: 2, startTime: 0,
    lastFrameTime: 0, // 新增：用于跟踪上一帧的时间，实现防挂机
    compileWaitFrames: 0, testMesh: null, originalRenderScale: 0.75, start: function(scene, camera, pathTracer, isMobile, onSuccess, onFail) { this.isTesting = true; this.testDone = false; this.state = 'idle';
    ...
    this.originalRenderScale = pathTracer.renderScale || (isMobile ? 0.42 : 0.75); pathTracer.renderScale = 0.1; const testGeo = new THREE.BoxGeometry(1, 1, 1); const testMat = new THREE.MeshPhysicalMaterial({ color: 0x888888 });
    ...
    window.hwLog("[PerfTest] 极简测试体(Box)已挂载并同步状态，等待 UI 渲染缓冲 (3帧)..."); }, update: function(pathTracer) { if (!this.isTesting || this.testDone) return; if (this.state === 'idle') { this.idleFrames++;
    if (this.idleFrames >= this.maxIdleFrames) { this.state = 'testing'; window.hwLog(`[PerfTest] 缓冲结束，正式向 GPU 发起光追测试指令 (目标: ${this.targetSamples}帧)...`); }
    return; }
    ...
    this.testDone = true; if (this.testMesh) { if (this.testMesh.parent) this.testMesh.parent.remove(this.testMesh); if (this.testMesh.geometry) this.testMesh.geometry.dispose(); if (this.testMesh.material) this.testMesh.material.dispose();
