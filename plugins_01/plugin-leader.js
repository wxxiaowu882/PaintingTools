window.AnnotationPluginManager.register({
    ...
    init: function() {
        ...
        viewer.addEventListener('pointerdown', (e) => { const modeSelect = document.getElementById('tool-mode-select'); if (modeSelect && modeSelect.value === 'leader' && e.altKey && e.shiftKey && e.button === 0) { e.preventDefault(); e.stopPropagation(); if (!this.isActiveDrawing) { const hit = viewer.positionAndNormalFromPoint(e.clientX, e.clientY); if (hit) { this.isActiveDrawing = true; this.startX = e.clientX; this.startY = e.clientY; const posStr = `${hit.position.x.toFixed(4)}m ${hit.position.y.toFixed(4)}m ${hit.position.z.toFixed(4)}m`; const normStr = `${hit.normal.x.toFixed(4)}m ${hit.normal.y.toFixed(4)}m ${hit.normal.z.toFixed(4)}m`; this.currentLeaderData = { id: pointIndex, type: 'leader', slot: `hotspot-leader-${pointIndex}`, pos: posStr, norm: normStr, text: `引线 ${pointIndex}`, dx: 0, dy: 0, color: defaultColor, hidden: false, hideInList: false, showTextOnLoad: true }; if (typeof window.addAnnotationToList === 'function') window.addAnnotationToList(this.currentLeaderData); else pointsData.push(this.currentLeaderData); this.mountDOM(this.currentLeaderData, viewer); const el = viewer.querySelector(`[slot="${this.currentLeaderData.slot}"]`); if(el) el.classList.add('show-text'); if(typeof updateSVG !== 'undefined') updateSVG(); } } } });
    ...
    mountDOM: function(p, viewer) {
        const el = document.createElement('div'); el.className = 'ink-anchor ink-mid'; el.setAttribute('slot', p.slot); el.setAttribute('data-position', p.pos); el.setAttribute('data-normal', p.norm); el.setAttribute('data-visibility-attribute', 'visible'); el.setAttribute('visible', ''); el.setAttribute('data-id', p.id); const dx = p.dx || 50; const dy = p.dy || -50; el.innerHTML = `<div class="HotspotAnnotation" style="left: 0 !important; top: -22px !important; transform: translate(${dx}px, ${dy}px); color: ${p.color||'#0df'};">${p.text}</div>`; const cid = p.id; el.addEventListener('click', function(evt) { if(window.tourState && window.tourState.isActive) return; this.classList.toggle('show-text'); if(typeof updateSVG !== 'undefined') updateSVG(); if(window.scrollToListItem) window.scrollToListItem(cid); }); viewer.appendChild(el);
    },
